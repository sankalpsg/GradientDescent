# -*- coding: utf-8 -*-
"""
Created on Wed Nov  1 18:36:29 2017
...
os.chdir("C:/Users/Sankalp/Downloads/")
...
def predict(X,theta):
  ...
  hypothesis = float(1) / (1 + np.e**(-X.dot(theta)))
  pred_value = np.where(hypothesis >= .5, 1, -1)
  return pred_value
...
def computeCost(X, y, theta):
  ...
  ##### replace the next line with your code #####
  y1 =  np.where(y==1,1,0)
  logistic_func = float(1) / (1 + np.e**(-X.dot(theta)))
  cost1 = y1 * np.log(logistic_func)
  cost0 = (1-y1) * np.log(1 - logistic_func)
  cost = (-cost1 - cost0)
  return np.mean(cost)
